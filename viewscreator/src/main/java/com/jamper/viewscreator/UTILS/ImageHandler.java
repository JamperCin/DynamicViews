...
import com.jamper.viewscreator.CALLBACK.OnImageSaved;
...
import static com.jamper.viewscreator.UTILS.CompressImage.getImageFromPath;
...
public class ImageHandler implements OnImageSaved {
    ...
    private void openLib(int pos) {
        switch (pos){
            case 0:  ImagePicker.create(mContext)
                    ...
                    .start(2000);
            break;
            case 1:  ImagePicker.create(mContext)
                    .returnAfterFirst(true) // set whether pick or camera action should return immediate result or not. For pick image only work on single mode
                    .single()  // single mode
                    .showCamera(false)
                    .start(2000);
...
*/
    ...
    private void printImages(List<Image> images) {
        ...
        Initialiser.getImageData().put(this.imageName, getBase64Bytes(bitmap));
